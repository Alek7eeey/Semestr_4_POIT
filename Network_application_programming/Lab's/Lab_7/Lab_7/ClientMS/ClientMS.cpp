...
void main()
{
	...
	try
	{
		if ((mailClient = CreateFile(TEXT("\\\\*\\mailslot\\Box"),
			GENERIC_WRITE,
		...
		{
			throw SetMailError("Error CreateFile: ", GetLastError());
		}
		...
		cout << "Сообщения успешно отправлены!" << endl;
		WriteFile(mailClient, ENDSEND, sizeEND, &wb, NULL);
