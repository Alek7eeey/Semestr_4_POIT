...
void PrintMatrix(CDC& dc, int x, int y, CMatrix& M)// Вывод объекта
// CDC -Класс объектов контекста устройства. Объект CDC обеспечивает функции для работы с контекстом устройства, типа дисплея
//связанным с клиентской областью окна
{

	// Для вывода элементов матрицы нужен тип CString
	// Так как мы не можем сразу преобразовать double в CString
	// Используем следующе переменные
	std::string dub_in_str;// Переменная для преобразования double (элемента массива) в строку
	char* str_in_char;// Переменная для преобразования string в *char, для конкатенации с CString и вывода в окно
	CString Str_Out = _T("");// _T - преобразование константы в длинную строку, Используется CString для вывода текста в функции TextOut
	...
	for (int i = 0; i < M.rows(); i++)
	{
		...
		{
			std::string _temp = std::to_string(M(i, j));
			if (_temp.length() > 3) {
				dub_in_str = _temp.substr(0, _temp.length() - 7).c_str();
			}
			else {
				dub_in_str = _temp.c_str();
			}

			str_in_char = (char*)dub_in_str.c_str();

			Str_Out += str_in_char;
			Str_Out += _T('  ');
		}
		dc.TextOutW(x, y + i * 20, Str_Out);
	}
}
...
// Методы с мат. опирациями над объектами 
//////////////..............////////////////
